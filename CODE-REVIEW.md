...
- `acc.subdirs.set(seg, ...)` (line 144) and `current.files.push(file)` (line 154) mutate data structures inside a reduce. A comment at line 125 explicitly acknowledges: *"Intentional mutation — immutable rebuild would require deep-cloning nested Maps on every insert"*. Should move to a recursive immutable builder.
...
- Two overlapping issues in the same function:
  1. `entry.page` (lines 49, 62, 77) and `colorIndex.value` (line 46) are mutated in-place. The JSDoc states *"Void; mutates entries in-place"*.
