---
title: Color
description: Color swatch display with click-to-copy.
---

import { Color, Columns, Column } from '@theme'

# Color

The `<Color>` component displays a color swatch with its value and an optional name. Click to copy the color value to your clipboard.

## Basic

**Code**

```mdx
import { Color } from '@theme'

<Color value="#7c3aed" />
<Color value="#2563eb" />
<Color value="#059669" />
```

**Output**

<Color value="#7c3aed" />
<Color value="#2563eb" />
<Color value="#059669" />

## With names

**Code**

```mdx
<Color value="#7c3aed" name="Brand Purple" />
<Color value="#2563eb" name="Info Blue" />
<Color value="#059669" name="Success Green" />
<Color value="#dc2626" name="Error Red" />
<Color value="#d97706" name="Warning Amber" />
```

**Output**

<Color value="#7c3aed" name="Brand Purple" />
<Color value="#2563eb" name="Info Blue" />
<Color value="#059669" name="Success Green" />
<Color value="#dc2626" name="Error Red" />
<Color value="#d97706" name="Warning Amber" />

## In a grid

**Code**

```mdx
import { Color, Columns, Column } from '@theme'

<Columns cols={3}>
  <Column><Color value="#7c3aed" name="Brand" /></Column>
  <Column><Color value="#2563eb" name="Info" /></Column>
  <Column><Color value="#059669" name="Success" /></Column>
</Columns>
```

**Output**

<Columns cols={3}>
  <Column><Color value="#7c3aed" name="Brand" /></Column>
  <Column><Color value="#2563eb" name="Info" /></Column>
  <Column><Color value="#059669" name="Success" /></Column>
</Columns>

## Props

| Prop    | Type     | Required | Default | Description                            |
| ------- | -------- | -------- | ------- | -------------------------------------- |
| `value` | `string` | yes      | —       | Color value (hex, rgb, hsl)            |
| `name`  | `string` | no       | —       | Display name for the color             |
